...
Dd('sbm_img').src = '<?php echo DT_SKIN;?>image/job_search.gif';
Dd('action').value = '';
} else if(id == 2) {
...
Dd('sbm_img').src = '<?php echo DT_SKIN;?>image/resume_search.gif';
...
&nbsp;&nbsp;
<a href="<?php echo $MOD['linkurl'];?><?php echo rewrite('search.php?action=job');?>" id="search_all" rel="nofollow"><img src="<?php echo DT_SKIN;?>image/btn_advance_search.gif"/></a>
...
<?php echo tag("moduleid=$moduleid&condition=status=3&areaid=$cityid&pagesize=".$MOD['page_ijob']."&length=20&order=".$MOD['order']."&template=table-job");?>
</div>
<div id="job_c_2" style="display:none">
<?php echo tag("moduleid=$moduleid&condition=status=3 and level>0&areaid=$cityid&pagesize=".$MOD['page_ijob']."&length=20&order=".$MOD['order']."&template=table-job");?>
</div>
</div>
<div class="b10">&nbsp;</div>
<?php } ?>
...
<?php echo tag("moduleid=$moduleid&table=resume&condition=status=3 and open=3&areaid=$cityid&showcat=1&pagesize=".$MOD['page_iresume']."&length=20&order=edittime desc&template=table-resume");?>
</div>
<div id="resume_c_2" style="display:none">
<?php echo tag("moduleid=$moduleid&table=resume&condition=status=3 and open=3 and level>0&areaid=$cityid&showcat=1&pagesize=".$MOD['page_iresume']."&length=20&order=edittime desc&template=table-resume");?>
</div>
</div>
<?php } ?>
</div>
...
<?php if($k%2==0) { ?><tr><?php } ?>
<td width="25%"><a href="<?php echo $MOD['linkurl'];?><?php echo $v['linkurl'];?>"><?php echo set_style($v['catname'],$v['style']);?></a></td>
<?php if($k%2==1) { ?></tr><?php } ?>
<?php } } ?>
</table>
</div>
