...
(function() {
	...
	$(window).bind("scroll.comment", function() {
		...
		if($(document).scrollTop() + cmh2 >= cmh1) {
			if($('#comment_main').html().toLowerCase().indexOf('<div>')!=-1) $('#comment_main').html('<if'+'rame src="'+DTPath+'api/comment.php?mid='+module_id+'&itemid='+item_id+'" name="xhxxfb_comment" id="des'+'toon_comment" style="width:99%;height:330px;" scrolling="no" frameborder="0"></if'+'rame>');
