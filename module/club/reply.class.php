...
class reply {
	...
	function add($post) {
		global $MOD, $table, $DT_TIME, $_username, $_passport;
		$post = $this->set($post);
		$sqlk = $sqlv = '';
		foreach($post as $k=>$v) {
		...
		$this->db->query("UPDATE {$table} SET replyuser='$_username',replyer='$_passport',replytime=$DT_TIME".($post['status'] == 3 ? ',reply=reply+1' : '')." WHERE itemid=$post[tid]");
